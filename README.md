...
- Public signup always creates `STUDENT` users; role input in signup payload is ignored.
- LMS module/lesson/progress endpoints enforce active enrollment (or admin/trainer ownership access).
- Razorpay webhook verification is computed from raw request bytes for signature correctness.
