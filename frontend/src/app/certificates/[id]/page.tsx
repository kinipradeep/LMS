...
export default async function CertificatePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const certificate = await api<Certificate>(`/certificates/${id}`);

  return (
    <section className="mx-auto max-w-3xl rounded-xl border bg-white p-8 shadow-sm">
      <p className="text-center text-sm uppercase tracking-[0.2em] text-slate-500">Certificate of Achievement</p>
